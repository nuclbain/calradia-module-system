...
def write_items(variable_list, variable_uses, tag_uses, quick_strings):
    item_kinds_file_name = export_dir + "item_kinds1.txt"
    open_file = open(item_kinds_file_name, "w")
    open_file.write("itemsfile version 3\n")
    open_file.write("%d\n" % len(items))
    for item in items:
        ...
        for item_variation in item_variations:
            open_file.write(" %s %d " % (item_variation[0], item_variation[1]))
        open_file.write(
        ...
        if len(item) > 9:
            ...
            for item_faction in item[9]:
                open_file.write(" %d" % item_faction)
            open_file.write("\n")
        else:
            open_file.write(" 0\n")
        trigger_list = []
